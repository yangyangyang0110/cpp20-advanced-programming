...
void test_non_blocking_with_callback(const fs::path &path) {
    const bool flag =
        non_blocking_by_callback::file2vector(path, [](std::string &&line) { std::cout << line << std::endl; });
}
...
int main() {
    const fs::path txt_path{"/home/ubuntu/workspace/cpp/"
                            "cpp20-advanced-programming/data/text/random.txt"};
