...
#define LOG_TRACE(msg, ...)                                                                        \
    spdlog::log({FILENAME_, __LINE__, __FUNCTION__}, spdlog::level::trace, msg, ##__VA_ARGS__)
#define LOG_DEBUG(msg, ...)                                                                        \
    spdlog::log({FILENAME_, __LINE__, __FUNCTION__}, spdlog::level::debug, msg, ##__VA_ARGS__)
#define LOG_INFO(msg, ...)                                                                         \
    spdlog::log({FILENAME_, __LINE__, __FUNCTION__}, spdlog::level::info, msg, ##__VA_ARGS__)
#define LOG_WARN(msg, ...)                                                                         \
    spdlog::log({FILENAME_, __LINE__, __FUNCTION__}, spdlog::level::warn, msg, ##__VA_ARGS__)
#define LOG_ERROR(msg, ...)                                                                        \
    spdlog::log({FILENAME_, __LINE__, __FUNCTION__}, spdlog::level::err, msg, ##__VA_ARGS__)
...
std::vector<fs::directory_entry> list_dir(const fs::path &p, bool is_sort_result = true)
{
    ...
    if (is_sort_result && !abs_paths.empty()) std::sort(abs_paths.begin(), abs_paths.end());
    return abs_paths;
}
...
std::optional<std::time_t> try_to_time_t(const std::string &datetime,
                                         const std::regex &reg = std::regex{
                                             R"((\d{4})-(\d{2})-(\d{2}))"})
{
    std::smatch pieces_match;
    if (!std::regex_match(datetime, pieces_match, reg)) return std::nullopt;

    const auto &&[year, month, day] = std::make_tuple(
        std::stoi(pieces_match[1]), std::stoi(pieces_match[2]), std::stoi(pieces_match[3]));
    ...
    const std::chrono::time_point<std::chrono::system_clock> &invalid_time_point_boundary)
{
    ...
    const std::time_t time = std::chrono::system_clock::to_time_t(invalid_time_point_boundary);
    ...
    for (const auto &dir : dirs) {
        ...
        if (!opt) continue;
        if (invalid_time_point_boundary > std::chrono::system_clock::from_time_t(opt.value())) {
            // std::cout << "remove-dirname: " << dir.path() << std::endl;
            detail::remove_directory(dir.path());
        }
    }
}
...
std::string join(const std::vector<std::string> &vec, const std::string &delim)
{
    ...
    std::for_each(vec.cbegin(), vec.cend(),
                  [&res, &delim](const std::string &item) { res.append(item + delim); });
    return res;
}
...
{
public:
    easy_file_sink(filename_t base_log_dir, filename_t log_basename, size_t max_file_bytes,
        ...
        , max_retention_days_(max_retention_days)
    {
        ...
        if (max_retention_days_ > 0) {
            ...
            ::helper::logger::detail::remove_invalid_log_dir(base_log_dir_,
                                                             now - max_retention_days * 24h);
        }
...
protected:
    void sink_it_(const details::log_msg &msg) override
    {
        ...
        if (time >= rotation_tp_) {
            ...
            auto target_name =
                gen_filename_by_filesize(base_log_dir_, log_basename_, now_tm(time),
...
private:
    ...
    void delete_old_()
    {
        for (auto iter = files_path_list_.begin(); iter != files_path_list_.end();) {
            if (files_path_list_.size() <= max_retention_days_) break;

            for (const auto &it : *iter) {
                bool ok = details::os::remove_if_exists(it) == 0;
                if (!ok)
                    throw_spdlog_ex(
                        "Failed removing daily file " + details::os::filename_to_str(it), errno);
            }
    ...
    static filename_t gen_filename_by_daily(const filename_t &base_log_dir,
                                            const filename_t &basename, const tm &now_tm)
    {
        auto date_dir_name = fmt::format("{:04d}-{:02d}-{:02d}", now_tm.tm_year + 1900,
                                         now_tm.tm_mon + 1, now_tm.tm_mday);
        const auto &&[basename_wo_ext, ext] = details::file_helper::split_by_extension(basename);
        return fmt::format(SPDLOG_FILENAME_T("{}{}{}"),
                           helper::logger::detail::join({base_log_dir, date_dir_name}, OS_SEP),
                                               ...
                                               const int32_t &idx)
    {
        ...
        return fmt::format(
            SPDLOG_FILENAME_T("{}{}{}{}"),
            helper::logger::detail::join({base_log_dir, date_dir_name}, OS_SEP), basename_wo_ext,
...
public:
    ...
    gen_logger(const spdlog::level::level_enum &level, const std::string &logger_name,
               const std::string &base_log_dir, size_t max_file_bytes, size_t max_retention_days)
    {
        ...
        if (max_file_bytes > 0 && max_retention_days > 0) {
            sinks.push_back(std::make_shared<spdlog::sinks::easy_file_sink_mt>(
                base_log_dir, logger_name, max_file_bytes, max_retention_days));
        }
    ...
    static void set_default_logger(const LogLevel &level, const std::string &logger_name,
                                   const std::string &base_log_dir, size_t max_file_bytes,
                                   size_t max_retention_days)
    {
        ...
        auto logger =
            gen_logger(spd_level, logger_name, base_log_dir, max_file_bytes, max_retention_days);
...
static void initializeGlobalLogger(const LogLevel &level, const std::string &base_log_dir,
                                   const std::string &log_filename, size_t max_file_size = 0,
                                   size_t max_retention_days = 0)
{
    static LoggerGenerator loggerGenerator;
    LoggerGenerator::set_default_logger(level, log_filename, base_log_dir,
                                        max_file_size * static_cast<int32_t>(fileSizeType),
                                        max_retention_days);
}
