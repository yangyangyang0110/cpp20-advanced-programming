...
#define LOG_TRACE(msg, ...)                \
  spdlog::log(                             \
      {FILENAME_, __LINE__, __FUNCTION__}, \
      spdlog::level::trace,                \
      msg,                                 \
      ##__VA_ARGS__)
...
std::vector<fs::directory_entry> list_dir(
    const fs::path& p, bool is_sort_result = true) {
...
std::optional<std::time_t> try_to_time_t(
    const std::string& datetime,
    const std::regex& reg = std::regex{R"((\d{4})-(\d{2})-(\d{2}))"}) {
  std::smatch pieces_match;
  if (!std::regex_match(datetime, pieces_match, reg))
    return std::nullopt;

  const auto&& [year, month, day] = std::make_tuple(
      std::stoi(pieces_match[1]),
...
std::string join(
  ...
  std::string res;
  std::for_each(
      vec.cbegin(), vec.cend(), [&res, &delim](const std::string& item) {
        res.append(item + delim);
      });
  return res;
}
...
class easy_file_sink final : public base_sink<Mutex> {
 public:
  easy_file_sink(
    ...
    auto filename =
        gen_filename_by_daily(base_log_dir_, log_basename_, now_tm(now));
 ...
 protected:
  void sink_it_(const details::log_msg& msg) override {
    ...
    if (time >= rotation_tp_) {
      file_helper_.close();
      auto filename =
          gen_filename_by_daily(base_log_dir_, log_basename_, now_tm(time));
      file_helper_.open(filename, false);
      current_size_ = file_helper_.size();
      rotation_tp_ = next_rotation_tp_();
      ...
      // failure.
      if (max_retention_days_ > 0 &&
          files_path_list_.size() > max_retention_days_)
        delete_old_();
    } else if (current_size_ >= max_file_bytes_) {
      file_helper_.close();
      auto src_name =
          gen_filename_by_daily(base_log_dir_, log_basename_, now_tm(time));
      auto target_name = gen_filename_by_filesize(
          base_log_dir_,
          log_basename_,
          now_tm(time),
 ...
 private:
  ...
  void delete_old_() {
    for (auto iter = files_path_list_.begin();
         iter != files_path_list_.end();) {
      ...
      for (const auto& it : *iter) {
        ...
        if (!ok)
          throw_spdlog_ex(
              "Failed removing daily file " + details::os::filename_to_str(it),
              errno);
      }
  ...
  /*  */
  static filename_t gen_filename_by_daily(
      const filename_t& base_log_dir,
      const filename_t& basename,
      const tm& now_tm) {
    auto date_dir_name = fmt::format(
        "{:04d}-{:02d}-{:02d}",
        now_tm.tm_year + 1900,
        now_tm.tm_mon + 1,
        now_tm.tm_mday);
    const auto&& [basename_wo_ext, ext] =
        details::file_helper::split_by_extension(basename);
    return fmt::format(
        SPDLOG_FILENAME_T("{}{}{}"),
  ...
  //
  static filename_t gen_filename_by_filesize(
      const filename_t& base_log_dir,
      const filename_t& basename,
      const tm& now_tm,
      const int32_t& idx) {
    ...
    return fmt::format(
        ...
        basename_wo_ext,
        fmt::format(
            "{:02d}{:02d}{:02d}", now_tm.tm_hour, now_tm.tm_min, now_tm.tm_sec),
        ext.empty() ? ".log" : ext);
  }

  static bool rename_file_(
...
inline std::shared_ptr<logger> easy_logger_mt(
  ...
  return Factory::template create<sinks::easy_file_sink_mt>(
      logger_name, filename, max_size, max_keep_days);
}
...
inline std::shared_ptr<logger> easy_logger_st(
    ...
    size_t max_size,
    size_t max_keep_days = -1) {
  return Factory::template create<sinks::easy_file_sink_st>(
      logger_name, filename, max_size, max_keep_days);
}
...
class LoggerGenerator {
 public:
  static spdlog::level::level_enum to_spdlog_level(
    ...
    return (it != log_level_to_spdlog_level.end()) ? it->second
                                                   : spdlog::level::debug;
  }

  static std::shared_ptr<spdlog::logger> gen_logger(
